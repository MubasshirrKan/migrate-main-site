...
function PathComparison() {
  ...
  const paths = {
    ...
    middle: {
      // Gentle wave (not a perfectly straight line) so the glow filter's bounding
      // box always has height — a 0-height bbox collapses the filter and makes the
      // line vanish once the morph settles.
      smart: "M 120 150 C 300 132, 400 132, 500 150 C 600 168, 700 168, 880 150",
    ...
    >
      ...
      <motion.div 
        ...
        animate={{ 
          ...
          background: isSmart
            ? "radial-gradient(ellipse 70% 50% at 50% 50%, rgba(37,211,102,0.15) 0%, rgba(0,0,0,0) 100%)"
        ...
        <svg viewBox="0 0 1000 300" className="w-full h-full overflow-visible" preserveAspectRatio="xMidYMid meet">
          <defs>
            <linearGradient id="smartGrad" x1="0" y1="0" x2="1" y2="0">
               ...
               <stop offset="50%" stopColor="#25D366" stopOpacity="0.9" />
          ...
          {/* Paths — three lines in the "Struggle" state collapse to a single
              WhatsApp-green line in the "MigrateProperly" state. */}
          {[paths.top, paths.middle, paths.bottom].map((pathObj, i) => {
            const isPrimaryLine = i === 1; // middle line is the single smart line
            return (
            <g key={i}>
              {/* Base Glowing Path — solid continuous line (stroke set as a
                  static prop so Framer never tries to tween between gradients,
                  which would drop the stroke and leave only the dashes). */}
              <motion.path
                initial={false}
                animate={{
                  d: isSmart ? pathObj.smart : pathObj.messy,
                  strokeWidth: isSmart ? 4 : 2,
                  opacity: isSmart ? (isPrimaryLine ? 1 : 0) : 1
                }}
                transition={{ duration: 1.5, ease: [0.16, 1, 0.3, 1] }}
                fill="transparent"
                stroke={isSmart ? "#25D366" : "url(#messyGrad)"}
                strokeLinecap="round"
                filter={isSmart ? "url(#glowSmart)" : "url(#glowMessy)"}
              />
              {/* Animated Flowing Highlight — travels along the solid base line */}
              <motion.path
                initial={false}
                animate={{
                  d: isSmart ? pathObj.smart : pathObj.messy,
                  strokeWidth: isSmart ? 3 : 1,
                  opacity: isSmart ? (isPrimaryLine ? 0.95 : 0) : 0.4,
