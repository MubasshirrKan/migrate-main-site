...
import { IdCard, GraduationCap, Plane, Wallet, MailOpen, FileSignature, Languages, Activity } from "lucide-react";
    ...
    >
        <motion.div style={{ transformStyle: 'preserve-3d', rotateY: counterRotation }}>
          <div style={{ transform: `rotateY(${-angle}deg)`, transformStyle: 'preserve-3d' }}>
              ...
              >
                  <div className="text-2xl md:text-3xl mb-2 drop-shadow-[0_0_10px_rgba(255,255,255,0.4)]">{doc.icon}</div>
...
export default function AIFileCheckingSection() {
  ...
  const documents = [
    { title: "Passport & IDs", icon: <IdCard className="w-8 h-8 md:w-10 md:h-10 text-blue-400" strokeWidth={1.5} /> },
    { title: "Academic Transcripts", icon: <GraduationCap className="w-8 h-8 md:w-10 md:h-10 text-blue-400" strokeWidth={1.5} /> },
    { title: "Visa Applications", icon: <Plane className="w-8 h-8 md:w-10 md:h-10 text-blue-400" strokeWidth={1.5} /> },
    { title: "Financial Proofs", icon: <Wallet className="w-8 h-8 md:w-10 md:h-10 text-blue-400" strokeWidth={1.5} /> },
    { title: "Recommendation Letters", icon: <MailOpen className="w-8 h-8 md:w-10 md:h-10 text-blue-400" strokeWidth={1.5} /> },
    { title: "Statement of Purpose", icon: <FileSignature className="w-8 h-8 md:w-10 md:h-10 text-blue-400" strokeWidth={1.5} /> },
    { title: "Language Tests", icon: <Languages className="w-8 h-8 md:w-10 md:h-10 text-blue-400" strokeWidth={1.5} /> },
    { title: "Medical Records", icon: <Activity className="w-8 h-8 md:w-10 md:h-10 text-blue-400" strokeWidth={1.5} /> },
  ];
        ...
        >
          <button className="relative px-10 py-5 rounded-full group outline-none overflow-hidden transition-all duration-500 hover:scale-105 active:scale-95 bg-white/5 border border-white/10 hover:border-white/20 hover:bg-white/10 hover:shadow-[0_8px_32px_rgba(37,99,235,0.2)] backdrop-blur-md hover:backdrop-blur-xl min-w-[280px] shadow-[0_4px_24px_rgba(0,0,0,0.5)]">
             ...
             <span className="relative z-10 text-white/90 group-hover:text-white font-medium text-[20px] tracking-wide flex items-center justify-center gap-3 drop-shadow-md transition-colors">
               Check Documents 
               <span className="text-2xl leading-none -mt-1 text-blue-300 group-hover:text-blue-200 group-hover:animate-pulse transition-colors drop-shadow-[0_0_10px_rgba(96,165,250,0.5)]">
                 ✨
               </span>
             </span>
